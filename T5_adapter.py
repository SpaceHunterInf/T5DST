...
def train(args, *more):
    ...
    model.add_adapter('b', set_active=True)
    model.add_adapter('c', set_active=True)
    model.add_adapter('base', set_active=True)
    model.add_adapter_fusion(ac.Fuse(ac.Stack('base','a'),ac.Stack('base','b'),ac.Stack('base','c')))

    fusion_setup = ac.Fuse(ac.Stack('base','a'),ac.Stack('base','b'),ac.Stack('base','c'))
    model.train_adapter_fusion(fusion_setup)
    adapter_names = ['a','b','c']
    #experimenting new composition
    ...
    for name in adapter_names:
        adapter_save_path = os.path.join(adapter_save_path, name)
        if not os.path.exists(adapter_save_path):
            os.mkdir(adapter_save_path)
        task.model.save_adapter(adapter_save_path, name)

    fusion_save_path = os.path.join(adapter_save_path, 'fusion')
    if not os.path.exists(fusion_save_path):
            os.mkdir(fusion_save_path)
    task.model.save_adapter_fusion(fusion_save_path, name)
